...
void zedit_save_to_disk(int zone_num)
{
   ...
   */
  fprintf(zfile, "#%d\n" "%s~\n" "%d %d %d ", zone_table[zone_num].number, (zone_table[zone_num].name && *zone_table[zone_num].name) ? zone_table[zone_num].name : "undefined", zone_table[zone_num].top, zone_table[zone_num].lifespan, zone_table[zone_num].reset_mode);
...
void zedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
      ...
      cyn, OLC_NUM(d), nrm, cyn, zone_table[OLC_ZNUM(d)].number, grn, nrm, nrm, OLC_ZONE(d)->name ? OLC_ZONE(d)->name : "<NONE!>", grn, nrm, yel, OLC_ZONE(d)->lifespan, grn, nrm, yel, OLC_ZONE(d)->top, grn, nrm, yel, OLC_ZONE(d)->reset_mode ? ((OLC_ZONE(d)->reset_mode == 1) ? "Reset when no players are in zone." : "Normal reset.") : "Never reset", nrm, grn, nrm, yel, buf1, nrm, grn, nrm);
  ...
  while (MYCMD.command != 'S') {
    ...
    switch (MYCMD.command) {
      case 'M':
        sprintf(buf2, "%sLoad %s [%s%d%s], Max : %d, %%load : %d", MYCMD.if_flag ? " then " : "", mob_proto[MYCMD.arg1].player.short_descr, cyn, mob_index[MYCMD.arg1].virtual, yel, MYCMD.arg2, MYCMD.arg4);
        break;
      case 'G':
        sprintf(buf2, "%sGive it %s [%s%d%s], Max : %d", MYCMD.if_flag ? " then " : "", obj_proto[MYCMD.arg1].short_description, cyn, obj_index[MYCMD.arg1].virtual, yel, MYCMD.arg2);
        break;
      case 'O':
        sprintf(buf2, "%sLoad %s [%s%d%s], Max : %d, %%load : %d", MYCMD.if_flag ? " then " : "", obj_proto[MYCMD.arg1].short_description, cyn, obj_index[MYCMD.arg1].virtual, yel, MYCMD.arg2, MYCMD.arg4);
        break;
      case 'E':
        sprintf(buf2, "%sEquip with %s [%s%d%s], %s, Max : %d", MYCMD.if_flag ? " then " : "", obj_proto[MYCMD.arg1].short_description, cyn, obj_index[MYCMD.arg1].virtual, yel, equipment_types[MYCMD.arg3], MYCMD.arg2);
        break;
      case 'P':
        sprintf(buf2, "%sPut %s [%s%d%s] in %s [%s%d%s], Max : %d", MYCMD.if_flag ? " then " : "", obj_proto[MYCMD.arg1].short_description, cyn, obj_index[MYCMD.arg1].virtual, yel, obj_proto[MYCMD.arg3].short_description, cyn, obj_index[MYCMD.arg3].virtual, yel, MYCMD.arg2);
        break;
      case 'R':
        sprintf(buf2, "%sRemove %s [%s%d%s] from room.", MYCMD.if_flag ? " then " : "", obj_proto[MYCMD.arg2].short_description, cyn, obj_index[MYCMD.arg2].virtual, yel);
        break;
      case 'D':
        sprintf(buf2, "%sSet door %s as %s.", MYCMD.if_flag ? " then " : "", dirs[MYCMD.arg2], MYCMD.arg3 ? ((MYCMD.arg3 == 1) ? "closed" : ((MYCMD.arg3 == 2) ? "locked" : ((MYCMD.arg3 == 3) ? "hidden and closed" : ((MYCMD.arg3 == 4) ? "locked and hidden" : "hidden only")))) : "open"
...
void zedit_disp_comtype(struct descriptor_data *d)
{
  ...
  sprintf(buf,
      ...
      "%sM%s) Load Mobile to room             %sO%s) Load Object to room\r\n"
          ...
          "What sort of command will this be? : ", grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn, nrm);
  send_to_char(buf, d->character);
  ...
  switch (OLC_MODE(d)) {
    ...
    case ZEDIT_CONFIRM_SAVESTRING:
      switch (*arg) {
        ...
        case 'n':
        case 'N':
          sprintf(buf, "OLC: %s finished editing zone info for room %d.", GET_NAME(d->character), OLC_NUM(d));
          mudlog(buf, 'G', COM_BUILDER, TRUE);
          cleanup_olc(d, CLEANUP_ALL);
          break;
        default:
