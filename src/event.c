...
EVENT(camp)
{
  ...
  if ((int) (CAUSER_CH->in_room) == (int) info) {
    ...
    sprintf(buf, "%s has camped in room #%d.", GET_NAME(CAUSER_CH), world[IN_ROOM(CAUSER_CH)].number);
  ...
  if (strcmp(sinfo->command, "conjure elemental") == 0) {
    ...
    switch (number(1, 20)) {
      ...
      case 6:
        mob = read_mobile(real_mobile(sinfo->vnum_list[0]), REAL | NOEQUIP);
        IS_ANIMATED(mob) = 1;
        sprintf(buf, "The skeleton of %s stands here.", corpsename);
        if (GET_ADESC(mob)) {
          FREE(GET_ADESC(mob));
        }
        GET_ADESC(mob) = strdup(buf);
        sprintf(buf, "the skeleton of %s", corpsename);
        break;
      case 7: /* zombie */
      case 8:
      case 9:
      ...
      case 11:
        mob = read_mobile(real_mobile(sinfo->vnum_list[1]), REAL | NOEQUIP);
        IS_ANIMATED(mob) = 1;
        sprintf(buf, "The zombie of %s stands here.", corpsename);
        if (GET_ADESC(mob)) {
          FREE(GET_ADESC(mob));
        }
        GET_ADESC(mob) = strdup(buf);
        sprintf(buf, "the zombie of %s", corpsename);
        break;
      case 12: /* spectre */
      case 13:
      case 14:
      case 15:
        mob = read_mobile(real_mobile(sinfo->vnum_list[2]), REAL | NOEQUIP);
        IS_ANIMATED(mob) = 1;
        sprintf(buf, "The spectre of %s stands here.", corpsename);
        if (GET_ADESC(mob)) {
          FREE(GET_ADESC(mob));
        }
        GET_ADESC(mob) = strdup(buf);
        sprintf(buf, "the spectre of %s", corpsename);
        break;
      case 16: /* vampire */
      case 17:
      case 18:
        mob = read_mobile(real_mobile(sinfo->vnum_list[3]), REAL | NOEQUIP);
        IS_ANIMATED(mob) = 1;
        sprintf(buf, "The vampire of %s stands here.", corpsename);
        if (GET_ADESC(mob)) {
          FREE(GET_ADESC(mob));
        }
        GET_ADESC(mob) = strdup(buf);
        sprintf(buf, "the vampire of %s", corpsename);
        break;
      case 19: /* wraith */
      case 20:
        mob = read_mobile(real_mobile(sinfo->vnum_list[4]), REAL | NOEQUIP);
        IS_ANIMATED(mob) = 1;
        sprintf(buf, "The wraith of %s stands here.", corpsename);
        if (GET_ADESC(mob)) {
          FREE(GET_ADESC(mob));
        }
        GET_ADESC(mob) = strdup(buf);
        sprintf(buf, "the wraith of %s", corpsename);
        break;
    }
