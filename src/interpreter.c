    ...
    case CON_NAME_CNFRM: /* wait for conf. of new name	 */
      ...
      if (*arg == 'y' || *arg == 'Y') {
        ...
        if (restrict_game_lvl) {
          ...
          safe_snprintf(buf, MAX_STRING_LENGTH, "Request for new char %s denied from %s (wizlock)", GET_NAME(d->character), GET_HOST(d->character));
          mudlog(buf, 'C', COM_IMMORT, TRUE);
          STATE(d) = CON_CLOSE;
          return;
        }
    ...
    case CON_PASSWORD: /* get pwd for known player	 */
      ...
      if (!*arg)
            ...
            if (crypto_pwhash_str(GET_ENCPASSWD(d->character), GET_PASSWD(d->character), strlen(GET_PASSWD(d->character)), crypto_pwhash_OPSLIMIT_INTERACTIVE, crypto_pwhash_MEMLIMIT_INTERACTIVE) == 0) {
              *GET_PASSWD(d->character) = '\0';
              ...
              if (d->character->player.host[0] != '\0') {
                safe_snprintf(buf, MAX_STRING_LENGTH, "Auto updated PW: %s [%s]", GET_NAME(d->character), GET_HOST(d->character));
              } else {
        ...
        if (isbanned(GET_HOST(d->character)) == BAN_SELECT && !PLR_FLAGGED(d->character, PLR_SITEOK)) {
          ...
          STATE(d) = CON_CLOSE;
          safe_snprintf(buf, MAX_STRING_LENGTH, "Connection attempt for %s denied from %s", GET_NAME(d->character), GET_HOST(d->character));
          mudlog(buf, 'C', COM_IMMORT, TRUE);
          return;
        }
        if (GET_LEVEL(d->character) < restrict_game_lvl) {
          ...
          STATE(d) = CON_CLOSE;
          safe_snprintf(buf, MAX_STRING_LENGTH, "Request for login denied for %s [%s] (wizlock)", GET_NAME(d->character), GET_HOST(d->character));
          mudlog(buf, 'C', COM_IMMORT, TRUE);
          return;
        }
        ...
        for (k = descriptor_list; k; k = k->next)
          if (k->original && (GET_IDNUM(k->original) == GET_IDNUM(d->character))) {
            ...
            REMOVE_BIT(AFF2_FLAGS(d->character), AFF2_SCRIBING | AFF2_CASTING | AFF2_MEMMING);
            STATE(d) = CON_PLAYING;
            safe_snprintf(buf, MAX_STRING_LENGTH, "%s [%s] has reconnected.", GET_NAME(d->character), GET_HOST(d->character));
            mudlog(buf, 'C', COM_IMMORT, TRUE);
            return;
          }
        ...
        for (tmp_ch = character_list; tmp_ch; tmp_ch = tmp_ch->next)
          if (!IS_NPC(tmp_ch) && GET_IDNUM(d->character) == GET_IDNUM(tmp_ch)) {
            if (!tmp_ch->desc) {
              ...
              safe_snprintf(buf, MAX_STRING_LENGTH, "%s has re-logged in ... disconnecting old socket.", GET_NAME(tmp_ch));
              mudlog(buf, 'C', COM_IMMORT, TRUE);
    ...
    case CON_QCLASS:
      ...
      if (*arg == '\0') {
        ...
        if (isbanned(GET_HOST(d->character)) == BAN_OUTLAW) {
          ...
          safe_snprintf(buf, MAX_STRING_LENGTH, "Outlaw ON for new char %s from [%s] (siteban)", GET_NAME(d->character), GET_HOST(d->character));
          mudlog(buf, 'M', COM_IMMORT, TRUE);
        }
        if (isbanned(GET_HOST(d->character)) == BAN_FROZEN) {
          result = PLR_TOG_CHK(d->character, PLR_FROZEN);
          safe_snprintf(buf, MAX_STRING_LENGTH, "Frozen ON for new char %s from [%s] (siteban)", GET_NAME(d->character), GET_HOST(d->character));
          mudlog(buf, 'M', COM_IMMORT, TRUE);
        }
