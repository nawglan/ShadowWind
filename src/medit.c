...
void medit_disp_action_menu(struct descriptor_data *d)
{
  ...
  send_to_char("[H[J", d->character);
  while (action) {
    sprintf(buf, "%d) A %d %d %s\r\n", d->edit_number2, action->chance, action->minpos, action->action);
    send_to_char(buf, d->character);
    d->edit_number2++;
    action = action->next;
  }
  sprintf(buf, "%d) Create New Action\r\n", d->edit_number2);
  send_to_char(buf, d->character);
  sprintf(buf, "%d) Purge Action\r\n", d->edit_number2 + 1);
  send_to_char(buf, d->character);
...
void medit_disp_equip_pos(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_WEARS; i++) {
    sprintf(buf, "%s%2d%s) %-20.20s  ", grn, i, nrm, where[i]);
    if (!(++columns % 2))
      strcat(buf, "\r\n");
    send_to_char(buf, d->character);
  }
...
void medit_disp_equip_menu(struct descriptor_data *d)
{
  ...
  send_to_char("[H[J", d->character);
  while (equip) {
    sprintf(buf, "%d) E %d %d %d %d\r\n", d->edit_number2, equip->pos, equip->chance, equip->vnum, equip->max);
    send_to_char(buf, d->character);
    d->edit_number2++;
    equip = equip->next;
  }
  sprintf(buf, "%d) Create New Equipment\r\n", d->edit_number2);
  send_to_char(buf, d->character);
  sprintf(buf, "%d) Purge Equipment\r\n", d->edit_number2 + 1);
  send_to_char(buf, d->character);
...
void medit_disp_mob_flags(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_MOB_FLAGS; i++) {
    sprintf(buf, "%s%2d%s) %-20.20s  %s", grn, i + 1, nrm, action_bits[i], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void medit_disp_aff_flags(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_AFF_FLAGS; i++) {
    sprintf(buf, "%s%2d%s) %-20.20s  %s", grn, i + 1, nrm, affected_bits[i], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void medit_disp_aff2_flags(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_AFF2_FLAGS; i++) {
    sprintf(buf, "%s%2d%s) %-20.20s  %s", grn, i + 1, nrm, affected_bits2[i], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void medit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf, "%sF%s) Position  : %s%s" "  %sG%s) Default   : %s%s\r\n"
    ...
    case MEDIT_CONFIRM_SAVESTRING:
      ...
      switch (*arg) {
        ...
        case 'Y':
          ...
          sprintf(buf, "OLC: %s finished editing mob %d", GET_NAME(d->character), OLC_NUM(d));
