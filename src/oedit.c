...
void oedit_setup_existing(struct descriptor_data *d, int real_num)
{
  ...
  *obj = obj_proto[real_num];
  ...
  obj->short_description = strdup(obj_proto[real_num].short_description ? obj_proto[real_num].short_description : "undefined");
  obj->description = strdup(obj_proto[real_num].description ? obj_proto[real_num].description : "undefined");
  obj->action_description = (obj_proto[real_num].action_description ? strdup(obj_proto[real_num].action_description) : NULL);
  ...
  for (counter = zone_table[zone_num].number * 100; counter <= zone_table[zone_num].top; counter++) {
    if ((realcounter = real_object(counter)) >= 0) {
      ...
      fprintf(fp, "#%d\n"
          ...
          "%s~\n"
          "%s~\n"
          "%d ", GET_OBJ_VNUM(obj), (obj->name && *obj->name) ? obj->name : "undefined", (obj->short_description && *obj->short_description) ? obj->short_description : "undefined", (obj->description && *obj->description) ? obj->description : "undefined", buf1, GET_OBJ_TYPE(obj));
      ...
      fprintf(fp, "\n"
          "%d %d %d %d %d\n"
          "%d %d\n", GET_OBJ_VAL(obj, 0), GET_OBJ_VAL(obj, 1), GET_OBJ_VAL(obj, 2), GET_OBJ_VAL(obj, 3), GET_OBJ_VAL(obj, 4), GET_OBJ_WEIGHT(obj), GET_OBJ_COST(obj));
...
void oedit_disp_aff_flags(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_AFF_FLAGS; i++) {
    sprintf(buf, "%s%2d%s) %-20.20s  ", grn, i + 1, nrm, affected_bits[i]);
    if (!(++columns % 2))
      strcat(buf, "\r\n");
...
void oedit_disp_aff2_flags(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_AFF2_FLAGS; i++) {
    sprintf(buf, "%s%2d%s) %-20.20s  ", grn, i + 1, nrm, affected_bits2[i]);
    if (!(++columns % 2))
      strcat(buf, "\r\n");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_container_flags_menu(struct descriptor_data *d)
{
  ...
  send_to_char("[H[J", d->character);
  sprintf(buf, "%s1%s) CLOSEABLE\r\n"
      "%s2%s) PICKPROOF\r\n"
      "%s3%s) CLOSED\r\n"
      "%s4%s) LOCKED\r\n"
      "Container flags: %s%s%s\r\n"
      "Enter flag, 0 to quit : ", grn, nrm, grn, nrm, grn, nrm, grn, nrm, cyn, buf1, nrm);
  send_to_char(buf, d->character);
}
...
void oedit_disp_extradesc_menu(struct descriptor_data *d)
{
  ...
  grn, nrm, yel, (extra_desc->keyword && *extra_desc->keyword) ? extra_desc->keyword : "<NONE>", grn, nrm, yel, (extra_desc->description && *extra_desc->description) ? extra_desc->description : "<NONE>", grn, nrm, buf1, grn, nrm);
  send_to_char(buf, d->character);
...
void oedit_liquid_type(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_LIQ_TYPES; counter++) {
    sprintf(buf, " %s%2d%s) %s%-20.20s %s", grn, counter, nrm, yel, drinks[counter], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_apply_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_APPLIES; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter, nrm, apply_types[counter], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_weapon_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < WEAPON_MAX; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter, nrm, weapon_types[counter], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_handed_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < WEAPON_HANDED; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s ", grn, counter, nrm, weapon_handed[counter]);
    if (!(++columns % 2))
      strcat(buf, "\r\n");
    send_to_char(buf, d->character);
  }
    ...
    if (spells[counter].spell_pointer)
      for (i = 0; i < NUM_CLASSES - 1; i++)
        if (spells[counter].min_level[i] <= GET_OBJ_VAL(OLC_OBJ(d), 0)) {
          sprintf(buf, "%s%3d%s) %s%-20.20s %s", grn, spells[counter].spellindex, nrm, yel, spells[counter].command, !(++columns % 3) ? "\r\n" : "");
          send_to_char(buf, d->character);
          break;
        }
  }
...
void oedit_disp_type_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_ITEM_TYPES; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter, nrm, item_types[counter], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_extra_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_ITEM_FLAGS; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm, extra_bits[counter], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_wear_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_ITEM_WEARS; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm, wear_bits[counter], !(++columns % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_slots_menu(struct descriptor_data * d)
{
  ...
  for (counter = 0; counter < NUM_WEARS; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s ", grn, counter + 1, nrm, worn_bits[counter]);
    if (!(++columns % 2))
      strcat(buf, "\r\n");
    send_to_char(buf, d->character);
  }
...
void oedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf, "%s7%s) Wear flags     : %s%s\r\n"
      "%s8%s) Weight         : %s%d\r\n"
      "%s9%s) Cost           : %s%d\r\n"
      "%sB%s) Timer          : %s%d\r\n"
  ...
  grn, nrm, cyn, buf1, grn, nrm, cyn, GET_OBJ_WEIGHT(obj), grn, nrm, cyn, GET_OBJ_COST(obj), grn, nrm, cyn, GET_OBJ_TIMER(obj), grn, nrm, cyn, GET_OBJ_VAL(obj, 0), GET_OBJ_VAL(obj, 1), GET_OBJ_VAL(obj, 2), GET_OBJ_VAL(obj, 3), grn, nrm, cyn, buf2, grn, nrm, grn, nrm, grn, nrm, grn, nrm, cyn, buf4, grn, nrm, cyn, buf3, grn, nrm, cyn, buf5, grn, nrm, cyn, GET_OBJ_SVAL(obj, 0), GET_OBJ_SVAL(obj, 1), GET_OBJ_SVAL(obj, 2), grn, nrm);
  send_to_char(buf, d->character);
    ...
    case OEDIT_CONFIRM_SAVESTRING:
      switch (*arg) {
        ...
        case 'Y':
          ...
          sprintf(buf, "OLC: %s finished editing obj %d", GET_NAME(d->character), OLC_NUM(d));
