...
/* Local Functions called externally */
SPECIAL(CastleGuard);
SPECIAL(James);
SPECIAL(cleaning);
SPECIAL(DicknDavid);
...
/* Local Functions used interally only */
static int member_of_staff(struct char_data *chChar);
static int member_of_royal_guard(struct char_data *chChar);
static struct char_data *find_npc_by_name(struct char_data *chAtChar, char *pszName, int iLen);
...
int member_of_staff(struct char_data * chChar)
{
  ...
  return (ch_num == CASTLE_ITEM(1) || (ch_num > CASTLE_ITEM(2) && ch_num < CASTLE_ITEM(15)) || (ch_num > CASTLE_ITEM(15) && ch_num < CASTLE_ITEM(18)) || (ch_num > CASTLE_ITEM(18) && ch_num < CASTLE_ITEM(30)));
}
...
int member_of_royal_guard(struct char_data * chChar)
{
  ...
  ch_num = GET_MOB_VNUM(chChar);
  return (ch_num == CASTLE_ITEM(3) || ch_num == CASTLE_ITEM(6) || (ch_num > CASTLE_ITEM(7) && ch_num < CASTLE_ITEM(12)) || (ch_num > CASTLE_ITEM(23) && ch_num < CASTLE_ITEM(26)));
}
...
struct char_data *get_victim(struct char_data * chAtChar)
{
  ...
  for (ch = world[chAtChar->in_room].people; ch; ch = ch->next_in_room)
    if (FIGHTING(ch) && member_of_staff(FIGHTING(ch)) && ++iNum_bad_guys == iVictim)
      return ch;
...
int banzaii(struct char_data * ch)
{
  ...
  if ((chOpponent = get_victim(ch))) {
    act("$n roars: 'Protect the Kingdom of Great King Welmar!  BANZAIIII!!!'", FALSE, ch, 0, 0, TO_ROOM);
...
int do_npc_rescue(struct char_data * ch_hero, struct char_data * ch_victim)
{
  ...
  for (ch_bad_guy = world[ch_hero->in_room].people; ch_bad_guy && (FIGHTING(ch_bad_guy) != ch_victim); ch_bad_guy = ch_bad_guy->next_in_room)
    ;
  if (ch_bad_guy) {
...
{
  if (IS_SET(i->obj_flags.wear_flags, ITEM_WEAR_TAKE) && ((GET_OBJ_TYPE(i) == ITEM_DRINKCON) || (GET_OBJ_COST(i) <= 10)))
    return TRUE;
  else
    return FALSE;
}
...
{
  int colorspray = find_spell_num("color spray");
  int harm = find_spell_num("harm");
  int fireball = find_spell_num("harm");
  int heal = find_spell_num("harm");
  ...
  switch (number(0, 8)) {
    ...
    case 7:
      ...
      DO_SPELL(&spells[fireball], fireball, 1, ch, GET_NAME(tch), MPCAST);
      break;
    default:
      if (!number(0, 1))
        DO_SPELL(&spells[heal], heal, 1, ch, GET_NAME(tch), MPCAST);
      break;
  }
...
/* Used by King Welmar */SPECIAL(king_welmar)
{

  static char *monolog[] = {"$n proclaims 'Primus in regnis Geticis coronam'.", "$n proclaims 'regiam gessi, subiique regis'.", "$n proclaims 'munus et mores colui sereno'.", "$n proclaims 'principe dignos'."};
...
/* Used by the Training Master */SPECIAL(training_master)
{
  ...
  if (!banzaii(ch) && !number(0, 2)) {
    if ((pupil1 = find_npc_by_name(ch, "Brian", 5)) && (pupil2 = find_npc_by_name(ch, "Mick", 4)) && (!FIGHTING(pupil1) && !FIGHTING(pupil2))) {
      if (number(0, 1)) {
        tch = pupil1;
        pupil1 = pupil2;
        pupil2 = tch;
      ...
      switch (number(0, 7)) {
        ...
        case 2:
          ...
          break;
        case 3:
          act("$N yells at $n, as he fumbles and drops his sword.", FALSE, pupil1, 0, ch, TO_NOTVICT);
          ...
          act("You yell at $n, as he fumbles, losing his weapon.", FALSE, pupil1, 0, ch, TO_VICT);
          break;
        case 4:
          ...
          act("You trick $n, and quickly slash him across his back.", FALSE, pupil1, 0, pupil2, TO_VICT);
          break;
        case 5:
        ...
        default:
          ...
          act("$n shows his pupils an advanced technique.", FALSE, ch, 0, 0, TO_ROOM);
          break;
      }
    }
  }
...
/* Complains if he finds any trash... */SPECIAL(James)
{
...
/* Picks up any trash she finds... */SPECIAL(cleaning)
{
  struct obj_data *i;
...
/* Routine CastleGuard */
/* Standard routine for ordinary castle guards */SPECIAL(CastleGuard)
{
...
/* Routine DicknDave */
/* Routine for the guards Dick and David */SPECIAL(DicknDavid)
{
...
/* Routine peter */
/* Routine for Captain of the Guards. */SPECIAL(peter)
{
  ...
  if (!(number(0, 3)) && (ch_guard = find_guard(ch)))
    switch (number(0, 5)) {
      ...
      default:
        act("$n looks at you.", FALSE, ch, 0, ch_guard, TO_VICT);
        act("$n looks at $N.", FALSE, ch, 0, ch_guard, TO_NOTVICT);
        act("$n growls: 'That chain mail looks rusty!  CLEAN IT !!!'", FALSE, ch, 0, ch_guard, TO_ROOM);
        act("You growl at $N.", FALSE, ch, 0, ch_guard, TO_CHAR);
        break;
    }
...
SPECIAL(jerry)
{
  ...
  if (!banzaii(ch) && !number(0, 2)) {
    if ((gambler1 = ch) && (gambler2 = find_npc_by_name(ch, "Michael", 7)) && (!FIGHTING(gambler1) && !FIGHTING(gambler2))) {
      if (number(0, 1)) {
        tch = gambler1;
        gambler1 = gambler2;
        gambler2 = tch;
      ...
      switch (number(0, 5)) {
        case 0:
          act("$n rolls the dice and cheers loudly at the result.", FALSE, gambler1, 0, gambler2, TO_NOTVICT);
        ...
        case 2:
          ...
          act("$n sighs loudly as $e gives you your rightful win.", FALSE, gambler1, 0, gambler2, TO_VICT);
          break;
        case 3:
          act("$n smiles remorsefully as $N's roll tops his.", FALSE, gambler1, 0, gambler2, TO_NOTVICT);
          act("You smile sadly as you see that $N beats you. Again.", FALSE, gambler1, 0, gambler2, TO_CHAR);
          act("$n smiles remorsefully as your roll tops his.", FALSE, gambler1, 0, gambler2, TO_VICT);
          break;
        case 4:
          act("$n excitedly follows the dice with his eyes.", FALSE, gambler1, 0, gambler2, TO_NOTVICT);
          act("You excitedly follow the dice with your eyes.", FALSE, gambler1, 0, gambler2, TO_CHAR);
          act("$n excitedly follows the dice with his eyes.", FALSE, gambler1, 0, gambler2, TO_VICT);
          break;
        default:
