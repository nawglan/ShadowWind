...
int member_of_staff(struct char_data *chChar) {
  ...
  return (ch_num == CASTLE_ITEM(1) || (ch_num > CASTLE_ITEM(2) && ch_num < CASTLE_ITEM(15)) ||
...
int member_of_royal_guard(struct char_data *chChar) {
  ...
  ch_num = GET_MOB_VNUM(chChar);
  return (ch_num == CASTLE_ITEM(3) || ch_num == CASTLE_ITEM(6) ||
...
/* Used by King Welmar */ SPECIAL(king_welmar) {
  ...
  static char *monolog[] = {"$n proclaims 'Primus in regnis Geticis coronam'.",
                            "$n proclaims 'regiam gessi, subiique regis'.",
                            "$n proclaims 'munus et mores colui sereno'.", "$n proclaims 'principe dignos'."};
...
/* Used by the Training Master */ SPECIAL(training_master) {
  ...
  if (!banzaii(ch) && !number(0, 2)) {
    if ((pupil1 = find_npc_by_name(ch, "Brian", 5)) && (pupil2 = find_npc_by_name(ch, "Mick", 4)) &&
...
SPECIAL(jerry) {
  ...
  if (!banzaii(ch) && !number(0, 2)) {
    if ((gambler1 = ch) && (gambler2 = find_npc_by_name(ch, "Michael", 7)) &&
      ...
      case 4:
        ...
        break;
      default:
        act("$n says 'Well, my luck has to change soon', as he shakes the dice.", FALSE, gambler1, 0, gambler2,
            TO_NOTVICT);
