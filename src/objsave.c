...
int Crash_delete_text(char *name)
{
  ...
  if (!(fl = fopen(filename, "r"))) {
    if (errno != ENOENT) { /* if it fails but NOT because of no file */
      sprintf(buf1, "SYSERR: deleting text file %s (1)", filename);
      perror(buf1);
    }
    return 0;
  }
  ...
  if (unlink(filename) < 0) {
    if (errno != ENOENT) { /* if it fails, NOT because of no file */
      sprintf(buf1, "SYSERR: deleting text file %s (2)", filename);
      perror(buf1);
    }
  }
  return (1);
...
int gen_receptionist(struct char_data * ch, struct char_data * recep, int cmd, char *arg, int mode)
{
  ...
  if (CMD_IS("rent")) {
    ...
    sprintf(buf, "{r$n tells you, '{REnjoy your stay.{r'{x");
    act(buf, FALSE, recep, 0, ch, TO_VICT);
    ...
    if (mode == RENT_FACTOR) {
      ...
      sprintf(buf, "%s has rented in room #%d", GET_NAME(ch), world[IN_ROOM(ch)].number);
      mudlog(buf, 'R', COM_IMMORT, TRUE);
    } else { /* cryo */
      ...
      sprintf(buf, "%s has cryo-rented in room #%d.", GET_NAME(ch), world[IN_ROOM(ch)].number);
      mudlog(buf, 'R', COM_IMMORT, TRUE);
    }
