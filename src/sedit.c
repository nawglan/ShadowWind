...
void sedit_compact_rooms_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; S_ROOM(shop, i) != -1; i++) {
    safe_snprintf(buf, MAX_STRING_LENGTH, "%2d - [%s%5d%s]  | %s", i, cyn, S_ROOM(shop, i), nrm, !(++count % 5) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
  safe_snprintf(buf, MAX_STRING_LENGTH, "\r\n"
      "%sA%s) Add a new room.\r\n"
...
void sedit_shop_flags_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_SHOP_FLAGS; i++) {
    safe_snprintf(buf, MAX_STRING_LENGTH, "%s%2d%s) %-20.20s   %s", grn, i + 1, nrm, shop_bits[i], !(++count % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void sedit_no_trade_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_TRADERS; i++) {
    safe_snprintf(buf, MAX_STRING_LENGTH, "%s%2d%s) %-20.20s   %s", grn, i + 1, nrm, trade_letters[i], !(++count % 2) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
...
void sedit_types_menu(struct descriptor_data *d)
{
  ...
  for (i = 0; i < NUM_ITEM_TYPES; i++) {
    safe_snprintf(buf, MAX_STRING_LENGTH, "%s%2d%s) %s%-20s%s  %s", grn, i, nrm, cyn, item_types[i], nrm, !(++count % 3) ? "\r\n" : "");
    send_to_char(buf, d->character);
  }
