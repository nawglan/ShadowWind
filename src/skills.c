...
ACMD(do_steal)
{
  ...
  if (str_cmp(obj_name, "coins")) {
    ...
    if (!(obj = get_obj_in_list_vis(vict, obj_name, vict->carrying))) {
      ...
      if (!obj) {
        ...
        if ((GET_POS(vict) > POS_STUNNED)) {
          ...
          improve_skill(ch, skillnum, SKUSE_AVERAGE);
          if (logg) {
            sprintf(logbuffer, "%s stole %s from %s", GET_NAME(ch), obj->short_description, GET_NAME(vict));
      ...
      if (AWAKE(vict) && (percent > GET_SKILL(ch, skillnum))) {
        ...
        improve_skill(ch, skillnum, SKUSE_AVERAGE);
        if (logg) {
          sprintf(logbuffer, "%s tried to steal %s from %s", GET_NAME(ch), obj->short_description, GET_NAME(vict));
          mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
          plog(logbuffer, ch, LVL_IMMORT);
        }
      } else { /* Steal the item */
    ...
    if (AWAKE(vict) && (percent > GET_SKILL(ch, skillnum))) {
      ...
      improve_skill(ch, skillnum, SKUSE_AVERAGE);
      if (logg) {
        sprintf(logbuffer, "%s tried to steal coins from %s", GET_NAME(ch), GET_NAME(vict));
        mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
        plog(logbuffer, ch, LVL_IMMORT);
      }
    } else {
      ...
      switch (number(0, 3)) {
        case 0: /* plat */
          ...
          if (coins > 0) {
            ...
            improve_skill(ch, skillnum, SKUSE_AVERAGE);
            if (logg) {
              sprintf(logbuffer, "%s stole %d plat from %s", GET_NAME(ch), coins, GET_NAME(vict));
              mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
              plog(logbuffer, ch, LVL_IMMORT);
            }
        ...
        case 1: /* gold */
          ...
          if (coins > 0) {
            ...
            improve_skill(ch, skillnum, SKUSE_AVERAGE);
            if (logg) {
              sprintf(logbuffer, "%s stole %d gold from %s", GET_NAME(ch), coins, GET_NAME(vict));
              mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
              plog(logbuffer, ch, LVL_IMMORT);
            }
          } else {
        ...
        case 2: /* silver */
          ...
          if (coins > 0) {
            ...
            improve_skill(ch, skillnum, SKUSE_AVERAGE);
            if (logg) {
              sprintf(logbuffer, "%s stole %d silver from %s", GET_NAME(ch), coins, GET_NAME(vict));
              mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
              plog(logbuffer, ch, LVL_IMMORT);
            }
          } else {
        ...
        case 3: /* copper */
          ...
          if (coins > 0) {
            ...
            improve_skill(ch, skillnum, SKUSE_AVERAGE);
            if (logg) {
              sprintf(logbuffer, "%s stole %d copper from %s", GET_NAME(ch), coins, GET_NAME(vict));
              mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
              plog(logbuffer, ch, LVL_IMMORT);
            }
          } else {
...
ACMD(do_applypoison)
{
  ...
  if (!*arg)
    ...
    if (!CAN_WEAR(obj, ITEM_WEAR_WIELD))
      ...
      sprintf(buf, "You smear poison all over %s %s.\r\n", AN(arg), arg);
      send_to_char(buf, ch);
      improve_skill(ch, skillnum, SKUSE_AVERAGE);
    } else {
      sprintf(buf, "Your attempt to poison %s %s failed.\r\n", AN(arg), arg);
      send_to_char(buf, ch);
      improve_skill(ch, skillnum, SKUSE_AVERAGE);
    }
  }
  ...
  for (i = list; i; i = i->next_in_room) {
    ...
    if (i->next_in_room) {
      if (i->next_in_room->next_in_room)
        ...
        sprintf(buf2, " and %s", GET_NAME(i->next_in_room));
    } else
