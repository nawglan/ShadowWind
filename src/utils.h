...
#define FREE(x)                                                                                                   \
  {                                                                                                               \
    char sTemp[256];                                                                                              \
    sprintf(sTemp, "MEM: Freeing %d bytes in %s on line %d in %s.", sizeof(x), __FUNCTION__, __LINE__, __FILE__); \
    log(sTemp);                                                                                                   \
    fflush(NULL);                                                                                                 \
    free(x);                                                                                                      \
  }
#define CREATE(result, type, number)                                                                          \
  {                                                                                                           \
    char sTemp[256];                                                                                          \
    sprintf(sTemp, "MEM: Allocating %d bytes in %s on line %d in %s.", sizeof(type) * (number), __FUNCTION__, \
            __LINE__, __FILE__);                                                                              \
    log(sTemp);                                                                                               \
    fflush(NULL);                                                                                             \
...
#define GET_NAME(ch) \
  (IS_ANIMATED(ch) ? (ch)->animated_name : (IS_NPC(ch) ? (ch)->player.short_descr : (ch)->player.name))
...
#define GET_INVIS_LEV(ch)     (IS_NPC(ch) ? GET_MOB_INVIS_LEV(ch) : GET_PLR_INVIS_LEV(ch))
...
#define CAN_WEARSPEC(obj, part, ch) (IS_SET((obj)->obj_flags.wear_flags, (part)) || COM_FLAGGED(ch, COM_ADMIN))
...
#define INVIS_OK_OBJ(sub, obj) (!IS_OBJ_STAT((obj), ITEM_INVISIBLE) || IS_AFFECTED((sub), AFF_DETECT_INVIS))
...
#define CAN_GET_OBJ(ch, obj) \
  (CAN_WEARSPEC((obj), ITEM_WEAR_TAKE, ch) && CAN_CARRY_OBJ((ch), (obj)) && CAN_SEE_OBJ((ch), (obj)))

#define PERS(ch, vict) (CAN_SEE(vict, ch) ? (GET_NAME(ch) ? GET_NAME(ch) : "UNKNOWN") : "someone")

#define OBJS(obj, vict)                                                                                            \
  (CAN_SEE_OBJ((vict), (obj)) ? ((obj)->cshort_description ? (obj)->cshort_description : (obj)->short_description) \
                              : "something")

#define OBJN(obj, vict) \
  (CAN_SEE_OBJ((vict), (obj)) ? ((obj)->cname ? fname((obj)->cname) : fname((obj)->name)) : "something")
...
#define IS_WAR(ch) (GET_CLASS(ch) == CLASS_WARRIOR || GET_CLASS(ch) == CLASS_MONK || GET_CLASS(ch) == CLASS_RANGER)
#define IS_THI(ch)                                                                                    \
  (GET_CLASS(ch) == CLASS_THIEF || GET_CLASS(ch) == CLASS_ASSASSIN || GET_CLASS(ch) == CLASS_ROGUE || \
   GET_CLASS(ch) == CLASS_MERCENARY || GET_CLASS(ch) == CLASS_BARD)
#define IS_PRI(ch)                                                                                   \
  (GET_CLASS(ch) == CLASS_PRIEST || GET_CLASS(ch) == CLASS_CLERIC || GET_CLASS(ch) == CLASS_DRUID || \
   GET_CLASS(ch) == CLASS_SHAMAN)
#define IS_MAGE(ch)                                                                                        \
  (GET_CLASS(ch) == CLASS_SORCERER || GET_CLASS(ch) == CLASS_WIZARD || GET_CLASS(ch) == CLASS_ENCHANTER || \
   GET_CLASS(ch) == CLASS_CONJURER || GET_CLASS(ch) == CLASS_NECROMANCER)

#define MAGE_CLASS(class)                                                                          \
  (((1 << CLASS_SORCERER) | (1 << CLASS_WIZARD) | (1 << CLASS_ENCHANTER) | (1 << CLASS_CONJURER) | \
    (1 << CLASS_NECROMANCER)) &                                                                    \
   (1 << (class)))

#define PRIEST_CLASS(class) \
  (((1 << CLASS_PRIEST) | (1 << CLASS_CLERIC) | (1 << CLASS_DRUID) | (1 << CLASS_SHAMAN)) & (1 << (class)))
...
#define IS_PRIEST(ch) (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_PRIEST))

#define IS_WARRIOR(ch) (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_WARRIOR))

#define IS_WIZARD(ch)      (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_WIZARD))
#define IS_MERCENARY(ch)   (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_MERCENARY))
#define IS_MONK(ch)        (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_MONK))
#define IS_CONJURER(ch)    (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_CONJURER))
#define IS_RANGER(ch)      (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_RANGER))
#define IS_ROGUE(ch)       (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_ROGUE))
#define IS_ASSASSIN(ch)    (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_ASSASSIN))
#define IS_CLERIC(ch)      (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_CLERIC))
#define IS_SHAMAN(ch)      (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_SHAMAN))
#define IS_SORCERER(ch)    (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_SORCERER))
#define IS_NECROMANCER(ch) (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_NECROMANCER))
#define IS_ENCHANTER(ch)   (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_ENCHANTER))
