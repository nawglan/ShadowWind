...
ACMD(do_quit)
{
  ...
  if (subcmd == 0 && GET_LEVEL(ch) < LVL_IMMORT) {
    ...
    safe_snprintf(buf, MAX_STRING_LENGTH, "%s has quit the game in room #%d. (died)", GET_NAME(ch), world[IN_ROOM(ch)].number);
  ...
  if (!IS_AFFECTED(ch, AFF_GROUP)) {
    ...
    if (IS_AFFECTED(k, AFF_GROUP)) {
      if (IN_ROOM(k) != IN_ROOM(ch)) {
        ...
        safe_snprintf(buf, MAX_STRING_LENGTH, "     {c[{W%3d{w/{W%3d{wH {G%3d{g/{G%3d{gM {B%3d{b/{B%3d{bV{c] {c[{W%2d %s{c]{x $N {R({WHead of group{R){x", GET_HIT(k), GET_MAX_HIT(k), GET_MANA(k), GET_MAX_MANA(k), GET_MOVE(k), GET_MAX_MOVE(k), GET_LEVEL(k), CLASS_ABBR(k));
    ...
    for (f = k->followers; f; f = f->next) {
      ...
      if (IN_ROOM(f->follower) != IN_ROOM(ch)) {
        ...
        safe_snprintf(buf, MAX_STRING_LENGTH, "     {c[{W%3d{w/{W%3d{wH {G%3d{g/{G%3d{gM {B%3d{b/{B%3d{bV{c] {c[{W%2d %s{c]{x $N", GET_HIT(f->follower), GET_MAX_HIT(f->follower), GET_MANA(f->follower), GET_MAX_MANA(f->follower), GET_MOVE(f->follower), GET_MAX_MOVE(f->follower), GET_LEVEL(f->follower), CLASS_ABBR(f->follower));
...
ACMD(do_report)
{
  ...
  one_argument(argument, arg);
  safe_snprintf(buff, sizeof(buff), "{wI have: {y%d{w/{y%dH{w, {y%d{w/{y%dM{w, {y%d{w/{y%dV{x", GET_HIT(ch), GET_MAX_HIT(ch), GET_MANA(ch), GET_MAX_MANA(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));
