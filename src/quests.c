    ...
    switch (quest->goal) {
      case GOAL_OBJECT:
        if (quest->value) {
          ...
          if (obj) {
            sprintf(logbuffer, "%s completed quest #%d, received %s (%d) as reward.", GET_NAME(ch), quest->qnum, (obj->cshort_description ? obj->cshort_description : (obj->short_description ? obj->short_description : "something")), quest->value);
      ...
      case GOAL_EXPERIENCE:
        if (quest->value) {
          sprintf(logbuffer, "%s completed quest #%d, received %d exp as reward.", GET_NAME(ch), quest->qnum, quest->value);
          mudlog(logbuffer, 'Q', COM_IMMORT, TRUE);
      ...
      case GOAL_SKILL:
        ...
        if (skillnum >= 0) {
          sprintf(logbuffer, "%s completed quest #%d, received %s skill as reward.", GET_NAME(ch), quest->qnum, quest->skillname);
          mudlog(logbuffer, 'Q', COM_IMMORT, TRUE);
