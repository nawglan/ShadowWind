...
void say_spell(struct char_data * ch, int spellnum, struct char_data * tch, struct obj_data * tobj)
{
  ...
  if (tch != NULL && tch != ch) {
    sprintf(buf1, "$n stares at you and utters the words, '%s'.", GET_CLASS(ch) == GET_CLASS(tch) ? get_spell_name(spellnum) : buf);
...
ACMD(do_spells)
{
  ...
  if (!*arg) {
    ...
    sprintf(buf2, "%ss can use the following spells:\r\n", buf);
