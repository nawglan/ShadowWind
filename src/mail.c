...
mail_index_type *find_char_in_index(char *searchee)
{
  ...
  if (searchee == 0) {
    stderr_log("SYSERR: Mail system -- non fatal error #1.");
    return 0;
  }
  for (tmp = mail_index; (tmp && strcasecmp(tmp->recipient, searchee)); tmp = tmp->next)
    ;
...
void write_to_file(void *buf, int size, long filepos)
{
  ...
  if (filepos % BLOCK_SIZE) {
    stderr_log("SYSERR: Mail system -- fatal error #2!!!");
    no_mail = 1;
    return;
  }
...
void read_from_file(void *buf, int size, long filepos)
{
  ...
  if (filepos % BLOCK_SIZE) {
    stderr_log("SYSERR: Mail system -- fatal error #3!!!");
    no_mail = 1;
    return;
  }
  fseek(mail_file, filepos, SEEK_SET);
...
char *read_delete(char *recipient)
 ...
 header (i.e. the text handed to the player) */
{
  ...
  if (!(mail_pointer = find_char_in_index(recipient))) {
    stderr_log("SYSERR: Mail system -- post office spec_proc error?  Error #7.");
    return 0;
  }
  if (!(position_pointer = mail_pointer->list_start)) {
    stderr_log("SYSERR: Mail system -- non-fatal error #8.");
    return 0;
  }
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  sprintf(buf, " * * * * Weirvane Mail System * * * *\r\n"
      "Date: %s\r\n"
      "  To: %s\r\n"
...
void postmaster_send_mail(struct char_data * ch, struct char_data *mailman, int cmd, char *arg)
{
  ...
  if (GET_GOLD(ch) < STAMP_PRICE && GET_LEVEL(ch) < LVL_IMMORT) {
    if (GET_BANK_GOLD(ch) >= STAMP_PRICE) {
  ...
  if (GET_LEVEL(ch) < LVL_IMMORT) {
    ...
    sprintf(buf, "$n tells you, 'Write your message. (/s saves /h for help)'");
  act(buf, FALSE, mailman, 0, ch, TO_VICT);
  SET_BIT(PLR_FLAGS(ch), PLR_MAILING | PLR_WRITING);
  sprintf(buf, "%s begins writing a mail message.", GET_NAME(ch));
...
void postmaster_check_mail(struct char_data * ch, struct char_data *mailman, int cmd, char *arg)
{
  ...
  if (has_mail(GET_NAME(ch)))
    sprintf(buf, "$n tells you, '{WYou have {Rmail {Wwaiting.{x'");
  else
    sprintf(buf, "$n tells you, '{WSorry, you don't have any mail waiting.{x'");
  act(buf, FALSE, mailman, 0, ch, TO_VICT);
}
...
void postmaster_receive_mail(struct char_data * ch, struct char_data *mailman, int cmd, char *arg)
{
  ...
  while (has_mail(GET_NAME(ch))) {
    ...
    GET_OBJ_COST(obj) = 30;
    obj->action_description = read_delete(GET_NAME(ch));
