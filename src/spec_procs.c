...
void show_skills(struct char_data * ch, struct char_data *vict)
{
  ...
  if (ch == vict)
    sprintf(buf, "You know of the following:\r\n\r\n{cSkills{C: {m-------{x\r\n");
  else
    sprintf(buf, "%s knows the following:\r\n\r\n{cSkills{C: {m-------{x\r\n", GET_NAME(vict));
...
void list_skills(struct char_data * ch)
{
  ...
  sprintf(buf2, "You know of the following:\r\n\r\n{cSkills{C: {m-------{x\r\n");
...
SPECIAL(weapon)
{
  ...
  if (skill_num < 1 || GET_LEVEL(ch) < spells[skill_num].min_level[(int) GET_CLASS(ch)] || (skill_num < skillnum)) {
    sprintf(buf, "What kind of combat technique is that?\r\n");
    send_to_char(buf, ch);
    return 1;
  }
...
SPECIAL(guild)
{
  ...
  if ((GET_SKILL(ch, spells[skill_num].spellindex) != 0) && spells[skill_num].spell_pointer) {
    sprintf(buf, "You have no need to learn that %s again.\r\n", SPLSKL(ch));
    send_to_char(buf, ch);
    return 1;
  }

  if (spells[skill_num].quest_only) {
    sprintf(buf, "I cannot teach you that %s.\r\n"
  ...
  if (PRACS_COST && prac_cost > (temp_gold + temp_bank_gold)) {
    sprintf(buf, "You cannot afford to practice that %s.\r\n", SPLSKL(ch));
    send_to_char(buf, ch);
    return 1;
  }
    ...
    switch (number(0, 3)) {
      case 0:
        ...
        if (coins > 0) {
          ...
          sprintf(logbuffer, "%s stole %d plat from %s", GET_NAME(ch), coins, GET_NAME(victim));
      ...
      case 1:
        ...
        if (coins > 0) {
          ...
          sprintf(logbuffer, "%s stole %d gold from %s", GET_NAME(ch), coins, GET_NAME(victim));
          mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
        }
        break;
      case 2:
        ...
        if (coins > 0) {
          ...
          sprintf(logbuffer, "%s stole %d silver from %s", GET_NAME(ch), coins, GET_NAME(victim));
          mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
        }
        break;
      case 3:
        ...
        if (coins > 0) {
          ...
          sprintf(logbuffer, "%s stole %d copper from %s", GET_NAME(ch), coins, GET_NAME(victim));
          mudlog(logbuffer, 'Y', COM_IMMORT, TRUE);
        }
        break;
    }
  ...
  if (CMD_IS("list")) {
    ...
    if (buying >= 1 && buying <= numspells) {
      if (GET_GOLD(ch) >= healer_costs[buying]) {
        sprintf(buf, "That will only cost you %d gold coins!\r\n", healer_costs[buying]);
        send_to_char(buf, ch);
...
SPECIAL(cityguard)
{
  ...
  if (HUNTING(ch) && GET_POS(ch) == POS_STANDING) {
    ...
    switch (number(0, 40)) {
      case 0:
        if (ch && tch) {
          sprintf(buf, "%s yells 'Stand arms guards, %s is a threat to the peace!!!'\r\n", GET_NAME(ch), GET_NAME(tch));
          zone_echo(ch, buf);
        }
        break;
      case 1:
        if (ch && tch) {
          sprintf(buf, "%s yells 'Come here %s, justice will be done!'\r\n", GET_NAME(ch), GET_NAME(tch));
          zone_echo(ch, buf);
        }
        break;
      case 2:
        if (ch && tch) {
          sprintf(buf, "%s yells 'You will be caught %s!!'\r\n", GET_NAME(ch), GET_NAME(tch));
          zone_echo(ch, buf);
        }
        break;
      case 3:
        if (ch && tch) {
          sprintf(buf, "%s yells 'Where are you %s, you cannot run forever!'\r\n", GET_NAME(ch), GET_NAME(tch));
          zone_echo(ch, buf);
        }
        break;
      case 4:
        if (ch && tch) {
          sprintf(buf, "%s yells '%s, you can run, but you can't hide!'\r\n", GET_NAME(ch), GET_NAME(tch));
          zone_echo(ch, buf);
        }
        break;
      default:
...
SPECIAL(slot_machine)
{
  ...
  if (CMD_IS("insert")) {
    ...
    if (amount > 0) {
      ...
      sprintf(buf, "You put the coins in the slot and pull the lever.\r\n");
      send_to_char(buf, ch);
      act("$n pulls the lever on the slot-machine.", TRUE, ch, 0, FALSE, TO_ROOM);
      if ((lucky = number(0, 100)) <= 1) {
        sprintf(buf, "You are a winner! You won %ld coins!\r\n", amount * 100);
        send_to_char(buf, ch);
        ...
        sprintf(buf, "You are a loser! You lost %ld coins!\r\n", amount);
        send_to_char(buf, ch);
        act("The lights on the slot-machine flash in brilliant patterns.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
      }
...
SPECIAL(bank)
{
  ...
  if (CMD_IS("balance")) {
    if (has_money)
      sprintf(buf, "Your current balance is {W%d platinum {Y%d gold {w%d silver {y%d copper{x.\r\n", GET_BANK_PLAT(ch), GET_BANK_GOLD(ch), GET_BANK_SILVER(ch), GET_BANK_COPPER(ch));
    else
    ...
    switch (type1[0]) {
      ...
      case 'p':
        ...
        sprintf(buf, "You deposit {W%d platinum{x.\r\n", amount);
        send_to_char(buf, ch);
      ...
      case 'g':
        ...
        sprintf(buf, "You deposit {Y%d gold{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
      case 'S':
      case 's':
        ...
        sprintf(buf, "You deposit {w%d silver{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
      case 'C':
      case 'c':
        ...
        sprintf(buf, "You deposit {y%d copper{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
    }
    ...
    switch (type1[0]) {
      ...
      case 'p':
        ...
        GET_TEMP_GOLD(ch) += amount * 1000;
        GET_BANK_PLAT(ch) -= amount;
        sprintf(buf, "You withdraw {W%d platinum{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
      case 'G':
      case 'g':
        ...
        GET_TEMP_GOLD(ch) += amount * 100;
        GET_BANK_GOLD(ch) -= amount;
        sprintf(buf, "You withdraw {Y%d gold{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
      case 'S':
      case 's':
        ...
        GET_TEMP_GOLD(ch) += amount * 10;
        GET_BANK_SILVER(ch) -= amount;
        sprintf(buf, "You withdraw {w%d silver{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
      case 'C':
      case 'c':
        ...
        GET_TEMP_GOLD(ch) += amount;
        GET_BANK_COPPER(ch) -= amount;
        sprintf(buf, "You withdraw {y%d copper{x.\r\n", amount);
        send_to_char(buf, ch);
        act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
        return 1;
    }
    ...
    switch (type2[0]) {
      ...
      case 'g':
        if (is_plat) {
          sprintf(buf, "You exchange {W%d platinum{x for {Y%d gold{x.\r\n", amount, amount * (10 - GET_OBJ_SVAL(obj, 0) / 2));
          send_to_char(buf, ch);
          ...
          sprintf(buf, "You exchange {w%d silver{x for {Y%d gold{x.\r\n", amount - (amount % (10 + GET_OBJ_SVAL(obj, 0))), amount / (10 + GET_OBJ_SVAL(obj, 0)));
          send_to_char(buf, ch);
          GET_GOLD(ch) += amount / (10 + GET_OBJ_SVAL(obj, 0));
          ...
          sprintf(buf, "You exchange {y%d copper{x for {Y%d gold{x.\r\n", amount - (amount % (100 + (10 * GET_OBJ_SVAL(obj, 0)))), amount / (100 + (10 * GET_OBJ_SVAL(obj, 0))));
          send_to_char(buf, ch);
      ...
      case 's':
        if (is_plat) {
          ...
          sprintf(buf, "You exchange {y%d copper{x for {w%d silver{x.\r\n", amount - (amount % (10 + GET_OBJ_SVAL(obj, 0))), amount / (10 + GET_OBJ_SVAL(obj, 0)));
          send_to_char(buf, ch);
          GET_SILVER(ch) += amount / (10 + GET_OBJ_SVAL(obj, 0));
...
SPECIAL(master_sin)
{
  ...
  if (!(time_info.day % 3) && (run == 0)) { /* execute spec_proc every third day */
    ...
    if (vict == master)
      return FALSE;
    sprintf(buf, "%s shouts, '%s! You have what rightfully belongs to me!'\r\n", GET_NAME(master), GET_NAME(vict));
    global_echo(buf);
    char_from_room(vict);
